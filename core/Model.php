...
abstract class Model
{
    ...
    public function loadData($data)
    {
        foreach ($data as $key => $value) {
            if (property_exists($this, $key)) {
                // Handle numeric types for empty strings to prevent TypeErrors
                if ($value === '' && $this->isNumericProperty($key)) {
                    $this->{$key} = null;
                    continue;
                }
                $this->{$key} = $value;
    ...
    private function isNumericProperty($property)
    {
